const API_URL_WEATHER = 'http://api.openweathermap.org/data/2.5/weather?'; //API End Point WEATHER
const API_URL_FORECAST = 'http://api.openweathermap.org/data/2.5/forecast?'; //API End Point FORECAST

const APP_ID = '9c369e3b767545a0a6138dd12f90d7c8'; //API KEY
const Q = 'Lima'
const ID = 3936456
...
function getWeatherByCity () {
    fetch(`${API_URL_WEATHER}id=${ID}&appid=${APP_ID}`)
...
function getWeatherByCoordinates (lat, lng) {
    fetch(`${API_URL_WEATHER}lat=${lat}&lon=${lng}&appid=${APP_ID}`)
        .then(function(response) {
            return response.json()
        })
        .then(function (data) {
            ...
            console.log(`Amanece: ${amanece.getHours()}:${amanece.getMinutes()<10?'0':''}${amanece.getMinutes()}`)
            //Calculando SunSet
            const sunset = data.sys.sunset
            let anochece = new Date(sunset*1000)
            console.log(`Anochece: ${anochece.getHours()}:${anochece.getMinutes()<10?'0':''}${anochece.getMinutes()}`)
